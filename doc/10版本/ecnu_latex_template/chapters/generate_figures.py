...
def gen_fig3_3():
    ...
    # 混淆矩阵（行=真实，列=预测）：53样本，49正确，4处误判
    # 误判：耐心→理论(1)、启发→互动(1)、互动→启发(1)、情感→耐心(1)
    raw = np.array([
        [12,  0,  0,  0,  0,  0,  0],  # 理论讲授型 (12)
        [ 1,  6,  0,  0,  0,  0,  0],  # 耐心细致型 (7)
        [ 0,  0,  8,  0,  1,  0,  0],  # 启发引导型 (9)
        [ 0,  0,  0,  6,  0,  0,  0],  # 题目驱动型 (6)
        [ 0,  0,  1,  0,  8,  0,  0],  # 互动导向型 (9)
        [ 0,  0,  0,  0,  0,  7,  0],  # 逻辑推导型 (7)
        [ 0,  1,  0,  0,  0,  0,  2],  # 情感表达型 (3)
    ], dtype=float)
    ...
    ax.set_title('测试集（N=53），准确率92.5%', fontsize=9,
                 color=DGRAY, pad=4)
    fig.tight_layout()
