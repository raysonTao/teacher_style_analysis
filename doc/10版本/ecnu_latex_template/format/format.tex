...
\setlength{\headheight}{18.52pt}
\addtolength{\topmargin}{-6.52pt}
%\rightmargin=35mm
%                                                                       %
...
% 为黑体（STHeiti）和楷体等CJK无衬线字体启用伪粗体，消除 "TU/STHeiti(0)/b/n undefined" 警告
% 使用 \AtBeginDocument 确保在 CTex 字体初始化之后执行，避免被覆盖
\AtBeginDocument{%
  \IfFontExistsTF{STHeiti}{%
    % 用 AutoFakeBold 重新声明 zhhei 族（即 \heiti 命令所用的族），
    % 消除 "TU/STHeiti(0)/b/n undefined" 警告
    \setCJKfamilyfont{zhhei}[AutoFakeBold=2]{STHeiti}%
    \setCJKsansfont[AutoFakeBold=2]{STHeiti}%
  }{}%
}

% 消除 "Font shape OMS/TimesNewRoman(0)/m/n undefined" 警告
% \textcircled 等命令需要 OMS 编码；Times New Roman 无 OMS 字形，须先声明族再声明形状，回退到 cmsy
\makeatletter
