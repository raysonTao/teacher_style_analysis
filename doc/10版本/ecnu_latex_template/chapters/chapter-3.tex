...
\begin{tikzpicture}[
  ...
  \node[layer=green!15, below=of L4]  (L3) {\textbf{第三层：融合分类层（Fusion \& Classification Layer）}\\[2pt]
    特征投影 → 跨模态注意力（6个$\alpha_{i\to j}$）→ BiLSTM时序建模 → 注意力池化 → 7类分类器};
...
\end{figure}
    ...
    \textcircled{4} 话语边界检测：基于以下规则判断话语单元结束： - 逻辑链完整（如"因为...所以..."结构完成） - 出现话题转换标记（"那么""接下来""现在"） - 单元时长超过上限（$\Delta t > 30$s）
...
\begin{tikzpicture}[
  ...
  \coordinate (branch) at ([yshift=-0.9cm]COARSE.south);

  \node[finebox=purple,
        below=0.9cm of COARSE, xshift=-3.45cm] (FQ)
    {\textbf{Q（提问）}\\[3pt]
     Heuristic-Q\\启发性提问\\[3pt]
     Factual-Q\\事实性提问};

  \node[finebox=orange,
        below=0.9cm of COARSE, xshift=-1.15cm] (FE)
    {\textbf{E（解释）}\\[3pt]
     Definition\\[2pt]
     Reasoning\\[2pt]
     Theory\\[2pt]
  ...
  \node[finebox=teal,
        below=0.9cm of COARSE, xshift=+1.15cm] (FI)
    {\textbf{I（指令）}\\[3pt]
     Organization\\组织指令\\[3pt]
     Task\\任务指令};

  \node[finebox=red,
        below=0.9cm of COARSE, xshift=+3.45cm] (FF)
    {\textbf{F（反馈）}\\[3pt]
  ...
  %% 分支箭头（树形）
  \draw[darr] (COARSE.south) -- (branch);
  \draw[darr] (branch) -| (FQ.north);
  \draw[darr] (branch) -| (FE.north);
  \draw[darr] (branch) -| (FI.north);
  \draw[darr] (branch) -| (FF.north);
...
\begin{tikzpicture}[
  ...
  %% 三模态输入列
  \node[box=gray]  (Fv) {视频特征\\$F_v\!\in\!\mathbb{R}^{20}$\\{\tiny ST-GCN}};
  \node[box=gray,  right=1.1cm of Fv] (Fa) {音频特征\\$F_a\!\in\!\mathbb{R}^{15}$\\{\tiny Wav2Vec 2.0}};
  \node[box=gray,  right=1.1cm of Fa] (Ft) {文本特征\\$F_t\!\in\!\mathbb{R}^{35}$\\{\tiny BERT+H-DAR}};
  ...
  \node[mbox=orange, below=0.7cm of Pa] (CMA)
        {跨模态注意力层\\$\alpha_{i\to j}=\mathrm{softmax}(Q_iK_j^T/\sqrt{d_k})$，共6个注意力权重};

  %% 时序建模
  \node[mbox=green, below=0.45cm of CMA] (LSTM)
        {BiLSTM时序建模\ \ $h_n=[\overrightarrow{h}_n;\overleftarrow{h}_n]\in\mathbb{R}^{1024}$};
