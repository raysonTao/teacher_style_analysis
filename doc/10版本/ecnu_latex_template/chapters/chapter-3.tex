...
本研究采用ST-GCN进行骨骼序列时序建模。ST-GCN将骨骼序列建模为时空图结构，通过图卷积捕捉关节间的依赖关系。相比单帧规则识别准确率提升17.7个百分点，推理速度快2.5倍，且骨骼表征具有隐私保护优势。
...
混淆矩阵（见图~\ref{fig:confusion-matrix}，7×7矩阵）用于分析类别间的混淆模式，预期混淆最多的类别对为[待分析后填入]。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{chapters/fig-3/fig-3-3.png}
\caption{SHAPE模型7类风格分类混淆矩阵（测试集，$N=53$，占位示例数据）}
...
\textbf{统计检验}：采用配对t检验（$\alpha = 0.05$）验证SHAPE与各基准方法的性能差异，SHAPE vs 早期融合：$t = \text{[待填]}$，$p = \text{[待填]}$；SHAPE vs 晚期融合：$t = \text{[待填]}$，$p = \text{[待填]}$。
...
\begin{itemize}
\item SHAP归因与注意力权重的Pearson相关系数：$r = \text{[待填]}$，$p = \text{[待填]}$
